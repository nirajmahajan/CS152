...
(for/list ([curr-strat strategies])
  (define keys (curr-strat key))
  (for/list ([curr-key keys])
    (cond [(not (equal? #f (algo:dictionary-closure (utils:add-substitution curr-key key))))
           (let* ([new-key (algo:dictionary-closure (utils:add-substitution curr-key key))]
                 [final-key (algo:secret-word-enumeration new-key)])
             (cond [(not (equal? #f final-key)) (utils:show-key final-key)]))]))))
