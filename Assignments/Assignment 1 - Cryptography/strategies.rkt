#lang racket

;; You can require more modules of your choice.
(require racket/list
         (prefix-in utils: "utils.rkt")
         (prefix-in stats: "statistics.rkt")
         "list-comprehension.rkt")

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;                                                                                     ;;
;; Strategies                                                                          ;;
;; ==========                                                                          ;;
;; For the purpose of this assignment, just the `etai` strategy is expected, since     ;;
;; we have dictionary-closure and secret-word-enumeration to leap-frog to the right    ;;
;; key. This technique would fail for harder keys which are arbitrary permutations of  ;;
;; the alphabet. We will be forced to explore many more strategies (along with         ;;
;; dictionary-closure of course).                                                      ;;
;;                                                                                     ;;
;; Strategies to guess substitutions for the key using statistical information about   ;;
;; - the English language from utils.rkt                                               ;;
;; - the cipher text      from statistics.rkt                                          ;;
;;                                                                                     ;;
;; Follow the function signature as indicated below. Deviations will make it           ;;
;; impossible for automatic grading of your submission.                                ;;
;; Moreover, we do not expect your strategies to require any more/different            ;;
;; arguments. Note that you recieve the key as argument, so you can at the very        ;;
;; least ensure that all the substitutions are monoalphabetic wrt this key.            ;;
;;                                                                                     ;;
;; Signature:                                                                          ;;
;; ```                                                                                 ;;
;; (define (my-fundoo-strategy key)                                                    ;;
;;   ;; Make use of `utils:ciphertext`, `utils:cipher-word-list`                       ;;
;;   ...)                                                                              ;;
;; ```                                                                                 ;;
;;                                                                                     ;;
;; Substitutions                                                                       ;;
;; -------------                                                                       ;;
;; In order to extend the key incrementally, we use `utils:add-substitution` to        ;;
;; extend a given key with a substitution.                                             ;;
;;                                                                                     ;;
;; A substitution is a list of pairs, each pair mapping a plaintext char to a          ;;
;; ciphertext char. For example, to extend the key with T -> a and O -> r              ;;
;; (simultaneously), we use the substitution:                                          ;;
;; ```                                                                                 ;;
;; (list (cons #\T #\a) (cons #\O #\r))                                                ;;
;; ```                                                                                 ;;
;; For a single substitution use a singleton list (containing just one pair).          ;;
;;                                                                                     ;;
;; **CAUTION**                                                                         ;;
;; -----------                                                                         ;;
;; 1. Note that add-substitution does not do sanity checks on the substitution and use ;;
;;    of `utils:is-monoalphabetic` is recommended to ensure that you don't             ;;
;;    inadvertently create invalid keys.                                               ;;
;; 2. You must provide a list called `compositions` in this module.                    ;;
;;                                                                                     ;;
;; See docs in "utils.rkt" for more information.                                       ;;
;;                                                                                     ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; You must add "public" functions of this module to this list.
(provide etai
         ;; Some more suggested strategies:
         
         ;; common-words-double
         ;; bigrams
         ;; common-initial-letters
         ;; common-final-letters
         ;; common-words-triple
         ;; trigrams
         ;; common-double-letters
         ;; common-words-quadruple
         ;; quadgrams
         
         ;; lists of strategies
         composition)

;; A strategy that uses some statistical information to generate potential
;; substitutions for E, T, A and I.
;; Refer the assignment manual for tips on developing this strategy. You can
;; interact with our etai with the executable we provide.
(define (etai key)
  (define letter-frequency (take (stats:cipher-monograms utils:ciphertext) 5))

  (define single-lettered (stats:cipher-common-words-single utils:cipher-word-list))
  (define double-lettered (stats:cipher-common-words-double utils:cipher-word-list))
  (define triple-lettered (stats:cipher-common-words-triple utils:cipher-word-list))

  (define freqlst-w/o-single-lettered (remove* single-lettered letter-frequency))

  (define most-frq-3-lettered (car triple-lettered))

  (define two-lettered-words-freq-list (stats:two-lettered-words-starting-with))

  ;; generates a code of length l using given list
  (define (code-generator l lst)
    (define (helper lst)
      (foldl (lambda (x y) (append y (permutations x)))
             '() lst))
  (helper (filter (lambda (x) (= (length x) l)) (combinations lst))))

 ; (define (move-sub-top sub lst)
  ;  (cons sub (remove sub lst)))

  ;; first split into cases whether both A and I are present in plaintext
  (cond [(= (length single-lettered) 2)
         (begin
           (define single1 (car single-lettered))
           (define single2 (cadr single-lettered))

           (define IA (list #\I #\A))
           (define ET (list #\E #\T))
           
           (define temp-ET (lc (stats:zip ET codes) : codes <- (code-generator 2 freqlst-w/o-single-lettered)))

           ;; possible ciphers for I and A (in correct order of probability)
           (define IA-codes (stats:sort-list-according-to (list single1 single2) two-lettered-words-freq-list))

           (define IA-final (lc (stats:zip IA codes) : codes <- (code-generator 2 IA-codes)))
           ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;to update;;;;;;;;;;;;;;;;;;;;;;;;;
           (define ET-final temp-ET)
           (lc (append x y) : x <- ET-final y <- IA-final))]
        [(= (length single-lettered) 1)
         (begin
           (define single1 (car single-lettered))
           (define I (list #\I))
           (define ETA (list #\E #\T #\A))
           (define A (list #\A))
           (define ETI (list #\E #\T #\I))
           
           ;; 2 cases - A is single1 - or - I is single1
           (define I-final (stats:zip I (list single1)))

           (define ETA-temp (lc (stats:zip ETA codes) : codes <- (code-generator 3 freqlst-w/o-single-lettered)))

           ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;to be updated;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
           (define ETA-final ETA-temp)

           (define case1-subs (append ETA-final I-final))

           (define A-final (stats:zip A (list single1)))

           (define ETI-temp (lc (stats:zip ETI codes) : codes <- (code-generator 3 freqlst-w/o-single-lettered)))

           ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;to be updated;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
           (define ETI-final ETI-temp)

           (define case2-subs (append ETI-final A-final))

           (list case1-subs case2-subs))]
        [else
         (begin
           (define ETAI (list #\E #\T #\A #\I))

           (define ETAI-final (lc (stats:zip ETAI codes) : codes <- (letter-frequency)))

           ETAI-final)]))
  

;; A suggested composition of strategies that might work well. Has not been
;; exhaustively tested by us. Be original ;)
(define composition (list etai))
;; common-words-double
;; bigrams
;; common-initial-letters
;; common-final-letters
;; common-words-triple
;; trigrams
;; common-double-letters))
;; common-words-quadruple
;; quadgrams))

